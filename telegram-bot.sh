...
#   /finalize-high-level     Approve current high-level plan, generate build plan
#   /approve-build           Write APPROVAL.md and trigger build
...
*Commands:*
/new-shot \<slug\> — Start new PoC (slug = lowercase-hyphenated)
/finalize-high-level — Approve plan & generate build plan
/approve-build — Approve build plan & start building
/status — Show current shot status
/cancel-shot — Cancel and clear active shot
/help — Show this message

*Workflow:*
1. /new-shot my-app
2. Send your idea text
3. Review plan, give feedback, or /finalize-high-level
4. /approve-build when ready
5. Get notified when live 🚀
EOF
)"
}
...
cmd_new_shot() {
  ...
  # Validate slug
  if [[ -z "$slug" ]]; then
    tg_send "$chat_id" "Usage: /new-shot \<slug\>\nExample: /new-shot my-cool-app"
    return
  fi

  if ! validate_slug "$slug"; then
    tg_send "$chat_id" "❌ Invalid slug. Use lowercase letters, numbers, and hyphens.\nExample: my-cool-app"
    return
  fi
...
cmd_approve_build() {
  ...
  local state; state="$(pipeline_state "$slug")"
  if [[ ! "$state" =~ ^(awaiting-approval|critique-pending|critiquing)$ ]]; then
  ...
  # Write approval
  cat > "$(artifact_file "$slug" "APPROVAL.md")" <<EOF
approved: yes
...
handle_message() {
  ...
  case "$cmd" in
    ...
    /new-shot)
      ...
      return
      ;;
    /finalize-high-level|/approve-high-level)
  ...
  case "$state" in
    ...
    building|testing|fixing|launching|build-planning|critiquing)
      ...
      ;;
    awaiting-approval)
      tg_send "$chat_id" "Use /approve-build to start building, or send more feedback."
      ;;
    done)
