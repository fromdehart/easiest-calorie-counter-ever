...
claude_run() {
  ...
  "$CLAUDE_BIN" -p "$prompt" --output-format text $extra_flags 2>&1
}
...
stage_init() {
  ...
  if [[ -d "$sdir" ]]; then
    fail "Directory already exists: ${sdir}. Delete it first or use a different slug."
  fi
...
PROMPT
  ...
  "$CLAUDE_BIN" -p "$prompt" --output-format text 2>&1 \
...
PROMPT
    ...
    tg_send "Reply with feedback or send /finalize-high-level to proceed."
...
PROMPT
  ...
  local plan_file; plan_file="$(artifact_path "$slug" "HIGH_LEVEL_PLAN.md")"
  cd "$sdir"
  "$CLAUDE_BIN" -p "$prompt" --output-format text 2>&1 > "$plan_file"

  tg_send "*[${slug}]* 📋 Updated plan:"
  tg_send "$(cat "$plan_file")"
  tg_send "Reply with more feedback or /finalize-high-level to proceed."
}
...
PROMPT
  ...
  "$CLAUDE_BIN" -p "$critique1_prompt" --output-format text 2>&1 \
...
PROMPT
  ...
  local combined; combined="$(cd "$sdir" && "$CLAUDE_BIN" -p "$revise_prompt" --output-format text 2>&1 \
...
PROMPT
  ...
  "$CLAUDE_BIN" -p "$critique2_prompt" --output-format text 2>&1 \
    | tee -a "${adir}/logs/critique.log" \
    > "$critique2_file"
...
PROMPT
  ...
  cd "$sdir"
  "$CLAUDE_BIN" -p "$build_prompt" --output-format text 2>&1 \
  ...
  if [[ ! -f ".convex/config.json" ]] && [[ ! -d ".convex" ]]; then
    info "Running convex dev --once to initialize project..."
    npx convex dev --once --configure=new \
      <<< "$slug" 2>&1 | tee -a "${adir}/logs/build.log" || \
    npx convex dev --once 2>&1 | tee -a "${adir}/logs/build.log" || true
  fi
...
PROMPT
    ...
    "$CLAUDE_BIN" -p "$fix_prompt" --output-format text 2>&1 \
...
stage_launch() {
  ...
  local convex_url=""
  if [[ -f ".env" ]]; then
    convex_url="$(grep '^VITE_CONVEX_URL=' .env | cut -d= -f2- | tr -d '"' || true)"
  fi

  # Deploy to Vercel (production)
  info "Running vercel --prod..."
  local vercel_output
  vercel_output="$(vercel --prod --yes 2>&1 | tee "${adir}/logs/launch.log")"
  ...
  # Set Vercel env vars if not already set
  if [[ -n "$convex_url" ]]; then
    echo "$convex_url" | vercel env add VITE_CONVEX_URL production 2>/dev/null | \
      tee -a "${adir}/logs/launch.log" || true
  fi
  echo "$slug" | vercel env add VITE_CHALLENGE_ID production 2>/dev/null | \
    tee -a "${adir}/logs/launch.log" || true

  # Re-deploy to pick up env vars
  vercel --prod --yes 2>&1 | tee -a "${adir}/logs/launch.log" || true
  deploy_url="$(vercel --prod --yes 2>&1 | grep -Eo 'https://[a-z0-9._-]+\.vercel\.app' | tail -1 || echo "$deploy_url")"
...
stage_full() {
  ...
  if [[ "$state" =~ ^(initialized|plan-failed)$ ]] || [[ ! -f "$(artifact_path "$slug" "HIGH_LEVEL_PLAN.md")" ]]; then
    if [[ "$mode" == "terminal" ]]; then
      ...
      info "Telegram mode: waiting for planning approval (send /finalize-high-level)"
      return 0
    fi
  fi
...
EOF
      ...
      info "Telegram mode: waiting for /approve-build command"
      return 0
    fi
  fi
